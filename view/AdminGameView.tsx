...
const AdminGameView: React.FC<AdminGameViewProps> = ({ game, onNext, resetGame }) => {
  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-full animate-in fade-in duration-700 overflow-hidden">
      
      {/* GAUCHE : VIDÉO (7/12) */}
      <div className="col-span-7 h-full">
        <div className="relative w-full h-full rounded-[3rem] overflow-hidden border-8 border-white/10 bg-black shadow-2xl">
          <iframe 
          ...
          {!revealed && (
            <div className="absolute inset-0 bg-indigo-950 flex flex-col items-center justify-center text-center p-4">
               ...
               <h2 className="text-5xl font-black text-white italic uppercase tracking-tighter">QUI ÉCOUTE ÇA ?</h2>
               <p className="text-yellow-400 mt-6 animate-pulse uppercase italic tracking-widest text-lg">En attente des votes...</p>
            </div>
          )}
        </div>
      </div>

      {/* DROITE : INFOS & RÉSULTATS (5/12) */}
      <div className="col-span-5 flex flex-col gap-4 h-full min-h-0">
        
        {/* Barre de Status */}
        <div className="shrink-0 flex justify-between items-center bg-indigo-900/80 p-4 rounded-2xl border-2 border-indigo-700 relative overflow-hidden">
          {!revealed && (
            <div className="absolute bottom-0 left-0 h-1.5 bg-yellow-400 transition-all duration-1000" 
          ...
          <div className="flex flex-col">
            ...
            <span className="text-white text-xl font-black italic">{game.currentTrackIndex + 1}/{game.shuffledPlaylist.length}</span>
          </div>
          <div className="text-center">
            <span className={`text-4xl font-black italic leading-none ${timeLeft < 6 && !revealed ? 'text-red-500 animate-pulse' : 'text-white'}`}>
              {revealed ? "STOP" : `${timeLeft}s`}
            </span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-yellow-400 text-xs font-black uppercase opacity-60">Votes</span>
            <span className="text-white text-xl font-black italic">{votes.length}/{game.participants.length}</span>
          </div>
        </div>

        {/* Panneau de résultats */}
        <div className="flex-1 min-h-0 bg-white p-6 rounded-[2.5rem] border-b-[12px] border-indigo-900 shadow-2xl flex flex-col overflow-hidden">
          {revealed ? (
            <div className="h-full flex flex-col">
              {/* Le Gagnant (Texte agrandi) */}
              <div className="text-center shrink-0 mb-4">
                <h3 className="text-indigo-900 font-black uppercase tracking-widest text-sm mb-2">C'ÉTAIT...</h3>
                <p className="text-4xl font-black text-white bg-gradient-to-r from-pink-500 to-orange-500 py-3 px-8 rounded-2xl shadow-xl border-4 border-white inline-block italic uppercase tracking-tighter">
                  {track.senderName}
                </p>
              </div>
              ...
              {/* Liste des votes (Scroll uniquement ici) */}
              ...
              {/* Actions (Boutons rétablis) */}
              <div className="shrink-0 flex flex-col gap-2">
                <button 
                  onClick={() => onNext()} 
                  className="w-full bg-yellow-400 hover:bg-yellow-300 text-indigo-900 font-black py-4 rounded-full text-2xl shadow-[0_6px_0_#b45309] active:translate-y-1 active:shadow-none transition-all uppercase italic border-4 border-white"
                >
            ...
            <div className="h-full flex flex-col items-center justify-center">
               <div className="text-9xl animate-bounce mb-6">🗳️</div>
               ...
               <div className="flex flex-wrap justify-center gap-3 max-w-sm">
                 {game.participants.map((_, i) => (
                   <div key={i} className={`w-6 h-6 rounded-full border-4 ${i < votes.length ? 'bg-green-500 border-green-600' : 'bg-gray-100 border-indigo-100'}`} />
                 ))}
               </div>
            </div>
          )}
