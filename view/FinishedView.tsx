...
interface FinishedViewProps {
  ...
  scores?: Record<
    string,
    { correctMade?: number; correctReceived?: number; submissions?: number }
  >;
  totalTracks?: number;
...
const FinishedView: React.FC<FinishedViewProps> = ({
  ...
  const entries =
    scores && Object.keys(scores).length > 0 ? Object.entries(scores) : [];

  const makeRanked = (
    entriesList: [string, any][],
    key: string,
    descending = true
  ) => {
    const arr = entriesList
      .slice()
      .map(([name, s]) => ({ name, stats: s, score: s[key] || 0 }));
    arr.sort(
      (a, b) =>
  ...
  return (
    <div className="h-full w-full bg-[#0f172a] flex flex-col items-center justify-start pt-6 px-4 pb-6 rounded-2xl text-center overflow-auto">
      ...
      <p className="text-indigo-300 mb-6">
        Résumé : {totalTracks} musiques jouées
      </p>
      {role === "admin" && scores && entries.length > 0 ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {bestGuessers.map((item, i) => {
                    ...
                    const precision =
                      totalTracks > 0
                        ? Math.round((correctMade / totalTracks) * 100)
                        : 0;
                    const rowClass =
                      item.rank === 1
                        ? "bg-yellow-500/20"
                    ...
                    return (
                      <tr
                        key={name}
                        className={`${rowClass} border-b border-white/5`}
                      >
                        <td className="p-2 font-bold text-white w-12">
                          {rankLabel}
                        </td>
        ...
        <p className="text-indigo-300">
          Les résultats sont visibles sur la TV !
        </p>
      )}
    </div>
  );
